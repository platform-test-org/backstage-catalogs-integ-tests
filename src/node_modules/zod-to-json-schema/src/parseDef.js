...
function parseDef(def, refs, preloadDefinitions) {
    const definitionSchemas = getPreloadedDefinitionSchemas(refs, preloadDefinitions);
    ...
    if (jsonSchema) {
        ...
        addDefinitionSchemas(jsonSchema, definitionSchemas, preloadDefinitions === null || preloadDefinitions === void 0 ? void 0 : preloadDefinitions.definitionsPath);
    }
...
const getPreloadedDefinitionSchemas = (refs, options) => options
    ? Object.entries(options.definitions).reduce((acc, [key, schema]) => {
        const jsonSchema = selectParser(schema._def, schema._def.typeName, refs);
        if (jsonSchema) {
            refs.items.push({
                def: schema._def,
    ...
    : undefined;
const addDefinitionSchemas = (jsonSchema, definitionSchemas, definitionsPath) => {
    if (definitionSchemas && definitionsPath !== undefined) {
        jsonSchema[definitionsPath] = Object.assign(Object.assign({}, definitionSchemas), jsonSchema[definitionsPath]);
    }
};
